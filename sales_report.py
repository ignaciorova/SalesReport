...
class Contacto:
    def __init__(self, nombre, cedula, puesto, tipo, warnings=None):
        ...
        if warnings is not None:
            if not pd.notna(nombre):
                warnings.append(f"Contacto sin nombre, asignado 'Desconocido'")
            if not pd.notna(tipo):
                warnings.append(f"Contacto {self.nombre} sin tipo, asignado 'Desconocido'")
...
class Venta:
    def __init__(self, cliente, empresa, fecha, orden, cantidad, precio_unitario, total, producto, vendedor, contacto, warnings=None):
        self.warnings = warnings if warnings is not None else []
        self.cliente = cliente if pd.notna(cliente) else "Desconocido"
        if not pd.notna(cliente):
            self.warnings.append(f"Venta sin cliente, asignado 'Desconocido'")
        ...
        try:
            ...
            self.fecha = None
            self.warnings.append(f"Venta con fecha inválida: {fecha}")
        
        self.orden = orden if pd.notna(orden) else ""
        self.cantidad = float(cantidad) if pd.notna(cantidad) else 0
        if not pd.notna(cantidad):
            self.warnings.append(f"Venta sin cantidad, asignado 0")
        
        self.precio_unitario = float(precio_unitario) if pd.notna(precio_unitario) else 0
        if not pd.notna(precio_unitario):
            self.warnings.append(f"Venta sin precio unitario, asignado 0")
        
        self.total = float(total) if pd.notna(total) else 0
        if not pd.notna(total):
            self.warnings.append(f"Venta sin total, asignado 0")
        
        self.producto = producto if pd.notna(producto) else "Producto Desconocido"
        if not pd.notna(producto):
            self.warnings.append(f"Venta sin variante de producto, asignado 'Producto Desconocido'")
        
        self.vendedor = vendedor if pd.notna(vendedor) else ""
        
        # Determinar tipo desde cliente o contacto
        self.tipo = ('BEN1_70' if 'BEN1_70' in client_parts[0] else
                     'BEN2_62' if 'BEN2_62' in client_parts[0] else
                     client_parts[0].replace('ASEAVNA ', '') if len(client_parts) > 0 else 'Desconocido')
        self.client_name = client_parts[1] if len(client_parts) > 1 else client_parts[0]

        self.contacto = contacto if contacto else Contacto(self.client_name, "Desconocido", "No especificado", self.tipo, self.warnings)
        self.name = self.contacto.nombre
        self.cedula = self.contacto.cedula
        self.position = self.contacto.puesto
        # Priorizar tipo del contacto si existe, con fallback al tipo del cliente
        self.tipo = self.contacto.tipo if self.contacto.tipo != "Desconocido" else self.tipo
        if self.tipo == "Desconocido":
            self.warnings.append(f"Venta para {self.client_name} sin tipo definido, asignado 'Desconocido'")

        # Asignar centro de costos
        cost_centers = {
            'BEN1_70': 'CostCenter_BEN1',
            'BEN2_62': 'CostCenter_BEN2',
            'AVNA VISITAS': 'CostCenter_Visitante',
            'Contratista/Visitante': 'CostCenter_Visitante',
    ...
    def aplicar_subsidios_y_comisiones(self, iva_rate):
        ...
        self.base_price = self.total / iva_factor if self.total > 0 else 0
        ...
        if self.is_subsidized:
            ...
            subsidy_rules = {
                'BEN1_70': {'subsidy': 2100, 'employee_payment': 1000, 'commission': 155},
                'BEN2_62': {'subsidy': 1800, 'employee_payment': 1300, 'commission': 150}
            }
            rule = subsidy_rules.get(self.tipo, {'subsidy': 0, 'employee_payment': self.total, 'commission': self.total * 0.05})
            self.subsidy = rule['subsidy']
            self.employee_payment = rule['employee_payment']
            self.employee_payment_base = self.employee_payment / iva_factor
            self.asoavna_commission = rule['commission']
            if self.tipo not in subsidy_rules:
                self.warnings.append(f"Producto subsidiado para tipo {self.tipo}, aplicado sin subsidio")
        else:
            self.subsidy = 0
            self.employee_payment = self.total
            self.employee_payment_base = self.base_price
            self.asoavna_commission = self.total * 0.05
            if self.producto == "Producto Desconocido":
                self.warnings.append(f"Producto no subsidiado desconocido, aplicada comisión del 5%")
...
class ReporteVentas:
    ...
    def _procesar_contactos(self, user_df):
        ...
        if missing_columns:
            self.warnings.append(f"Columnas faltantes en users_data.csv: {', '.join(missing_columns)}")
        ...
        for _, row in user_df.iterrows():
            contacto = Contacto(row['Nombre'], row['Cédula'], row['Puesto'], row['Tipo'], self.warnings)
    ...
    def _procesar_ventas(self, sales_df):
        ...
        if missing_columns:
            self.warnings.append(f"Columnas faltantes en sales_data.csv: {', '.join(missing_columns)}")
            raise ValueError(f"Columnas faltantes en sales_data.csv: {', '.join(missing_columns)}")

        ventas = []
        for _, row in sales_df.iterrows():
            cliente = row['Cliente']
            client_parts = cliente.split(', ') if pd.notna(cliente) else ['Desconocido']
            client_name = client_parts[1] if len(client_parts) > 1 else client_parts[0]
            normalized_client_name = Contacto(client_name, None, None, None, self.warnings).normalized_name
            ...
            if venta.fecha and venta.total != 0:
                ...
                self.warnings.append(f"Venta omitida para cliente {venta.cliente}: fecha inválida o total 0")
        return ventas
...
def main():
    ...
    warnings = reporte.get_warnings()
    if warnings:
        st.warning("Problemas detectados en los datos:")
        for warning in warnings:
            st.write(f"- {warning}")

    # Validar columnas
    expected_columns = ['client', 'display_name', 'name', 'company', 'date', 'order', 'quantity', 'unit_price', 'total', 'base_price', 'product', 'seller', 'cedula', 'position', 'tipo', 'cost_center', 'is_subsidized', 'subsidy', 'employee_payment', 'employee_payment_base', 'asoavna_commission', 'iva', 'client_credit', 'aseavna_account', 'warnings']
    missing_columns = [col for col in expected_columns if col not in sales_data.columns]
    if missing_columns:
        st.error(f"Faltan columnas en datos procesados: {', '.join(missing_columns)}")
        return
    ...
    if st.session_state.sort_key not in filtered_data.columns:
        st.error(f"Columna '{st.session_state.sort_key}' no existe. Usando 'display_name'.")
        st.session_state.sort_key = 'display_name'
    ...
    with tabs[1]:
        ...
        total_subsidy_filtered = sum(d['subsidy'] for d in facturacion_filtered.values())
        total_employee_payment_filtered = sum(d['employee_payment'] for d in facturacion_filtered.values())
        total_iva_filtered = sum(d['iva'] for d in facturacion_filtered.values())
        total_commission_subsidized_filtered = sum(d['commission'] for d in facturacion_filtered.values())
        ...
        facturacion_df = pd.DataFrame([
            {'': 'Subsidio', 'BEN1_70': 2100 if facturacion_filtered['BEN1_70']['count'] > 0 else 0, 'BEN2_62': 1800 if facturacion_filtered['BEN2_62']['count'] > 0 else 0},
            {'': 'Diferencia', 'BEN1_70': 1000 if facturacion_filtered['BEN1_70']['count'] > 0 else 0, 'BEN2_62': 1300 if facturacion_filtered['BEN2_62']['count'] > 0 else 0},
            {'': 'Iva', 'BEN1_70': facturacion_filtered['BEN1_70']['iva'], 'BEN2_62': facturacion_filtered['BEN2_62']['iva']},
            {'': 'PAGAR a colaborador', 'BEN1_70': 1000 if facturacion_filtered['BEN1_70']['count'] > 0 else 0, 'BEN2_62': 1300 if facturacion_filtered['BEN2_62']['count'] > 0 else 0},
            {'': 'Precio con iva', 'BEN1_70': 3100 if facturacion_filtered['BEN1_70']['count'] > 0 else 0, 'BEN2_62': 3100 if facturacion_filtered['BEN2_62']['count'] > 0 else 0},
            {'': '% Subsidio', 'BEN1_70': f"{subsidy_percentage_ben1_filtered:.2f}%", 'BEN2_62': f"{subsidy_percentage_ben2_filtered:.2f}%"},
            {'': 'Facturar a AVNA', 'BEN1_70': facturacion_filtered['BEN1_70']['subsidy'], 'BEN2_62': facturacion_filtered['BEN2_62']['subsidy']},
            {'': 'Monto a cobrar al trabajador', 'BEN1_70': facturacion_filtered['BEN1_70']['employee_payment'], 'BEN2_62': facturacion_filtered['BEN2_62']['employee_payment']},
            {'': 'Total', 'BEN1_70': total_ben1_filtered, 'BEN2_62': total_ben2_filtered},
            {'': 'Aseavna colones', 'BEN1_70': 155 if facturacion_filtered['BEN1_70']['count'] > 0 else 0, 'BEN2_62': 150 if facturacion_filtered['BEN2_62']['count'] > 0 else 0},
            {'': 'Aseavna %', 'BEN1_70': '5,0%' if facturacion_filtered['BEN1_70']['count'] > 0 else '0,0%', 'BEN2_62': '5,0%' if facturacion_filtered['BEN2_62']['count'] > 0 else '0,0%'},
        ])

        facturacion_df['Total'] = facturacion_df.apply(lambda row: (row['BEN1_70'] + row['BEN2_62']) if isinstance(row['BEN1_70'], (int, float)) else '', axis=1)
        facturacion_df.loc[facturacion_df[''] == 'Total', 'BEN1_70'] = facturacion_filtered['BEN1_70']['count']
        facturacion_df.loc[facturacion_df[''] == 'Total', 'BEN2_62'] = facturacion_filtered['BEN2_62']['count']
        ...
        facturacion_adicional_df = pd.DataFrame([
            {'': 'Facturar a AVNA', 'BEN1_70': format_number(facturacion_filtered['BEN1_70']['subsidy']), 'BEN2_62': format_number(facturacion_filtered['BEN2_62']['subsidy']), 'Total': format_number(total_facturar_avna)},
            ...
            {'': 'Facturar a ASEAVNA', 'BEN1_70': format_number(facturacion_filtered['BEN1_70']['employee_payment']), 'BEN2_62': format_number(facturacion_filtered['BEN2_62']['employee_payment']), 'Total': format_number(total_facturar_aseavna)},
            {'': 'Aseavna 5% (subsidios)', 'BEN1_70': format_number(facturacion_filtered['BEN1_70']['commission']), 'BEN2_62': format_number(facturacion_filtered['BEN2_62']['commission']), 'Total': format_number(total_aseavna_5percent_subsidized)},
            {'': 'Facturar a ASEAVNA (final)', 'BEN1_70': format_number(facturacion_filtered['BEN1_70']['employee_payment'] - facturacion_filtered['BEN1_70']['commission']), 'BEN2_62': format_number(facturacion_filtered['BEN2_62']['employee_payment'] - facturacion_filtered['BEN2_62']['commission']), 'Total': format_number(total_facturar_aseavna_final)},
        ])
    ...
    with tabs[4]:
        ...
        if st.session_state.selected_client != 'All':
            ...
            if client_data:
                ...
                st.subheader("Transacciones Subsidiadas")
                subsidized_df = client_data['subsidized'][['date', 'product', 'quantity', 'total', 'subsidy', 'employee_payment', 'asoavna_commission', 'client_credit', 'aseavna_account', 'iva', 'warnings']]
                ...
                non_subsidized_df = client_data['non_subsidized'][['date', 'product', 'quantity', 'total', 'subsidy', 'employee_payment', 'asoavna_commission', 'client_credit', 'aseavna_account', 'iva', 'warnings']]
    ...
    with tabs[5]:
        ...
        if not filtered_comisiones.empty:
            ...
            st.write("No hay transacciones de productos no subsidiados.")
                ...
                with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                    ...
                    if st.session_state.export_options['individual_report']:
                        for client, datos in reportes_individuales.items():
                            client_df = datos['transacciones'][['date', 'product', 'quantity', 'total', 'subsidy', 'employee_payment', 'asoavna_commission', 'client_credit', 'aseavna_account', 'iva', 'warnings']]
